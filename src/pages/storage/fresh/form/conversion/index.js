...
const mapDispatchToProps = dispatch => {
  return {
    ...
    getFreshItemStock: () => {
      dispatch({
        type: 'storage/GET_FRESH_ITEM_STOCK',
      })
    },
    ...
    setFreshItemStock: freshItemStock => {
      dispatch({
...
class ProcessConversion extends React.Component {
  ...
  componentDidMount() {
    const { generateConversionId, getFreshItemStock } = this.props
    generateConversionId()
    getFreshItemStock()
  }
  ...
  onInputItemChange = event => {
    ...
    const item = freshItemStock.filter(data => {
      return data.item_name === event
    })

    // Populate Input item list
    const outputItemData = freshItemStock.filter(data => {
  ...
  onFinish = values => {
    const { freshItemStock, setFreshItemStock } = this.props
    ...
    const updatedfreshItemStock = this.updatefreshItemStockQuantity(
      freshItemStock,
    ...
    setFreshItemStock(updatedfreshItemStock)
  ...
  updatefreshItemStockQuantity = (itemStock, itemName, quantityWeight, quantityVolume) => {
  ...
  render() {
    ...
    const { conversionId, freshItemStock } = this.props
    ...
    if (freshItemStock.length > 0) {
      const inputItemData = freshItemStock.filter(data => {
        return (
          data.group === 'whole' &&
          data.type === 'main' &&
